...
for (i in 1:10) {
  ...
  training_errors[i] <- mean(train_predicted_classes != train_data$Level)
}
...
traindf$Level <- as.factor(traindf$Level)
levels(traindf$Level) <- make.names(levels(traindf$Level))

model <- train(
  as.factor(Level) ~ Age + Gender + Air.Pollution + Genetic.Risk + OccuPational.Hazards + Smoking,
  data = traindf,
  method = "treebag",  
...
#testing
bag.te <- randomForest(as.factor(Level) ~ Age + Gender + Air.Pollution + Genetic.Risk + OccuPational.Hazards + Smoking, data = testdf,  mtry = p, importance=TRUE)
bag.te
